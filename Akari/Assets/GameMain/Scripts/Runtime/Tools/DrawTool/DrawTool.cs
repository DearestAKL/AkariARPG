...
public class DrawTool : MonoBehaviour
{
    ...
    public static void DrawRectangle2D(Transform t, float distance, float length, float width)
    {
        ...
        if (MathUtility.IsFacingRight(t))
        {
    ...
    private static GameObject CreateMesh(List<Vector3> vertices,Transform parent)
    {
        int[] triangles;
        Mesh mesh = new Mesh();
        ...
        if (go == null)
        {
            ...
            go.transform.position = new Vector3(parent.position.x, 0.1f, parent.position.y);//让绘制的图形上升一点，防止被地面遮挡
